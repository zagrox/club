...
@section('page-script')
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      // Initialize Rich Text Editor with improved configuration
      const quill = new Quill('#message-editor', {
        ...
        placeholder: 'متن پیام اعلان خود را اینجا بنویسید...',
        theme: 'snow',
        formats: [
      ...
      // Make sure Quill editor is in RTL mode for Persian/Arabic text
      ...
      document.getElementById('preview-btn').addEventListener('click', function(e) {
        ...
        if (!title || quill.getText().trim().length === 0) {
          Swal.fire({
            title: 'خطا!',
            text: 'عنوان و متن پیام برای پیش‌نمایش ضروری هستند',
            icon: 'error',
        ...
        // Update the hidden message field with the HTML content
        ...
        .catch(error => {
          ...
          Swal.fire({
            title: 'خطا!',
            text: 'خطایی در ایجاد پیش‌نمایش رخ داد. لطفا دوباره تلاش کنید.',
            icon: 'error',
            confirmButtonText: 'باشه'
          });
        });
      ...
      document.getElementById('save-draft-btn').addEventListener('click', function(e) {
        ...
        if (!title || quill.getText().trim().length === 0) {
          Swal.fire({
            title: 'خطا!',
            text: 'عنوان و متن پیام برای ذخیره پیش‌نویس ضروری هستند',
            icon: 'error',
            confirmButtonText: 'باشه'
          });
          return;
      ...
      notificationForm.addEventListener('submit', function(e) {
        ...
        if (!title || quill.getText().trim().length === 0) {
          Swal.fire({
            title: 'خطا!',
            text: 'عنوان و متن پیام برای ارسال اعلان ضروری هستند',
            icon: 'error',
            confirmButtonText: 'باشه'
          });
          return;
        ...
        if (deliveryMethods.length === 0) {
          Swal.fire({
            title: 'خطا!',
            text: 'حداقل یک روش ارسال باید انتخاب شود',
            icon: 'error',
            confirmButtonText: 'باشه'
          });
          return;
        ...
        if (!sendNow) {
          ...
          if (!scheduledAt) {
            Swal.fire({
              title: 'خطا!',
              text: 'برای زمان‌بندی، تاریخ و زمان ارسال باید مشخص شود',
              icon: 'error',
              confirmButtonText: 'باشه'
            });
            return;
          ...
          <div class="col-md-8 col-12 mb-4">
            <div class="card notification-card h-100">
              ...
              <div class="card-body">
                ...
                <div class="mb-4">
                  <label for="message-editor" class="form-label">متن پیام <span class="text-danger">*</span></label>
                  <div class="card border shadow-none">
                    <div id="message-editor" class="border-0" style="min-height: 250px;">{{ old('message') }}</div>
                  </div>
                  <div class="form-text">از ویرایشگر برای قالب‌بندی متن پیام خود استفاده کنید</div>
                  @error('message')
                    <div class="text-danger mt-1 small">{{ $message }}</div>
                  @enderror
                </div>
